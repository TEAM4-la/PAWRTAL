    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-4 mx-3 mt-4 rounded-2xl bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-100 flex items-center justify-between">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-400 to-emerald-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 overflow-hidden">
            {user?.avatar_url ? (
            ...
            <Link key={label} to={createPageUrl(page)} onClick={() => setOpen(false)}>
              <div
                className={`relative ml-2 flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group cursor-pointer
                  ...
                  ${active
                    ? 'bg-gradient-to-r from-teal-500 to-emerald-500 text-white shadow-md shadow-teal-200 before:w-1 before:bg-teal-500'
                    : 'text-teal-800 hover:bg-teal-50 hover:text-teal-900 before:w-0 before:bg-transparent'
                  }`}
              >
                <Icon
                  className={`flex-shrink-0 ${active ? 'text-white' : 'text-teal-500 group-hover:text-teal-700'}`}
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-teal-100 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to={createPageUrl('Settings')}>
              <div className="w-8 h-8 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold text-sm overflow-hidden border border-teal-200">
                {user?.avatar_url ? (
                  <img src={user.avatar_url} alt="Profile" className="w-full h-full object-cover" />
                ) : (
                  user?.full_name?.[0] || 'V'
